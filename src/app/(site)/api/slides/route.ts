...
export async function POST(request: NextRequest) {
  try {
    ...
    const { documentId, slideCount = 8, language = 'fr', theme = 'modern', includeImages = true, presentationStyle = 'professional' } = await request.json()
...
7. ALL TEXT MUST BE IN ${targetLanguage}`
    ...
    if (actualPageCost === 0) throw new Error('AI returned no slides')
